...
async function runTests() {
  ...
  await testCase(
    "POST /authenticate/login with admin credentials",
    async () => {
      const { response, data } = await callApi("/authenticate/login", {
        method: "POST",
        body: JSON.stringify({
          username: "test_user_1737925065455",
          password: 'test123',
        }),
      });
      ...
      assert(data.data.roles.includes("user"), "User role should be present");
      assert(!data.data.roles.includes("admin"), "Admin role should not be present");
